...
//            if viewModel.isDateRangeSheetPresented {
//                Text("This text is shown!")
//            }
...
struct TNDWBBottomSheet<Content: View>: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            if isShowing {
                ...
                .clipShape(TNDWBRoundedCorner(radius: topCornerRadius, corners: [.topLeft, .topRight]))
...
private struct TNDWBRoundedCorner: Shape {
    var radius: CGFloat
...
struct TNDWBMultiDatePickerView: View {
    ...
    private var legacyRangePreview: some View {
        if #unavailable(iOS 16.0) {
            HStack(spacing: 8) {
                ...
                Text(TNDWBRangeCalculator.rangeTitle(from: draftRangeStart, to: draftRangeEnd))
    ...
    private func syncLegacy(endingOn date: Date) {
        let window = TNDWBRangeCalculator.alignedWindow(containing: date, dayCount: selectionDayCount)
...
@available(iOS 16.0, *)
struct TNDWBCalendarIOS16: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UICalendarView {
        ...
        cv.availableDateRange = TNDWBRangeCalculator.calendarAvailableDateInterval()
    ...
    final class Coordinator: NSObject, UICalendarSelectionMultiDateDelegate {
        ...
        init(parent: TNDWBCalendarIOS16) { self.parent = parent }
        ...
        func applyAlignedRange(containing date: Date, updateBindings: Bool) {
            ...
            let w = TNDWBRangeCalculator.calendarWeekWindow(containing: tapped)
        ...
        private func applySelection(applicableStart: Date, applicableEnd: Date, updateBindings: Bool, animated: Bool) {
            let display = TNDWBRangeCalculator.calendarWeekDisplayWindow(containing: applicableEnd)
            let components = TNDWBRangeCalculator.allDates(from: display.start, to: display.end)
